...
async function availability({
  ...
  if (!start || !end || nightsBetween(start, end) <= 0) {
  ...
  const sql = `
    SELECT
      ...
      rt.name,
      rt.capacity_guests,
      rt.price_foreigner_usd,
      rt.price_resident_tzs,
      COALESCE(ri.total_units, 0) AS total_units,

      COALESCE(
        SUM(
          CASE
            WHEN b.status IN ('pending', 'confirmed', 'checked_in')
            THEN 1
            ELSE 0
          END
        ),
  ...
  return result.rows.map((r) => ({
    ...
    available_units: Math.max(
      0,
      Number(r.total_units) - Number(r.reserved_units)
    ),
...
async function createBooking(input) {
  ...
  return withTransaction(async (client) => {
    ...
    /*
     * First find the room type.
     *
     * IMPORTANT:
     * We do NOT use FOR UPDATE on this LEFT JOIN query.
    ...
    /*
     * Lock the actual room_types row separately.
     *
    ...
    if (guestsCount > Number(room.capacity_guests)) {
      throw new Error('ROOM_CAPACITY_EXCEEDED');
    }

    /*
     * Check existing reservations for the requested dates.
     */
    ...
    const reserved = Number(
      reservedResult.rows[0].reserved_units
    );

    const totalUnits = Number(room.total_units);

    if (totalUnits <= reserved) {
      throw new Error('NO_AVAILABILITY');
    }

    /*
     * Create or record the guest.
     */
    ...
    const isForeign =
      String(input.currency || 'TZS').toUpperCase() === 'USD';

    const nightlyRate = Number(
      isForeign
        ? room.price_foreigner_usd
        : room.price_resident_tzs
    );
     ...
     */
    const reference =
      `ELCT-${new Date().getUTCFullYear()}-${Math.floor(
